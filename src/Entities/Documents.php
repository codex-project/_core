...
class Documents extends ExtendableCollection implements Contracts\Documents\Documents, Arrayable
{
     ...
     *
     * @param \Codex\Entities\Ref $parent
     * @param \Codex\Codex        $codex
     */
    public function __construct($parent, Codex $codex)
    {
        parent::__construct();
        $this->ref     = $parent;
    ...
    public function resolveAll()
    {
        ...
        foreach ($files as $file) {
            ...
            $rel = path_relative($file, $this->ref->path());
            $rel2 = path_without_extension($rel);

            $file = Str::removeLeft($file, $this->ref->getName() . DIRECTORY_SEPARATOR);
            $file = Str::removeRight($file, '.' . path_get_extension($file));
            $rel = path_get_filename_without_extension($rel);

            $this->resolvePathName($file);
        }
    }
    ...
    public function resolve($pathName = null)
    {
        ...
        if (!$this->items->has($pathName)) {
            $document = $this->getCodex()->getContainer()->makeWith($resolved[ 'binding' ], [
                'codex'    => $this->getCodex(),
    ...
    public function toArray()
    {
        ...
        return $this->getItems()->map(function (\Codex\Entities\Document $document) {
